...
Product
======================================-->
<?php foreach($dataShowProductsRecommended as $recommendedItems): ?>
...
<div class="ps-product">
    ...
    <!-- IMAGE ******************************* -->
    <!-- *************************************  -->
    <a href="<?=$path . $recommendedItems->url_product?>">
        <img 
        src="img/products/<?=$recommendedItems->url_category?>/<?=$recommendedItems->image_product?>" 
        alt="<?=$recommendedItems->url_product?>"
        />
    </a>

    <?php
        $strPercent = "";
        if(!is_null($recommendedItems->offer_product))
        {
            $offer = json_decode(
                $recommendedItems->offer_product,
                true
            );
            $strPercent = HelperController::offerDiscountPercentage(
                $recommendedItems->price_product,
                $offer[1],
                $offer[0]
            );
    ...
    <div class="ps-product__container">
        ...
        <a class="ps-product__vendor" href="<?=$path . $recommendedItems->url_store?>">
            <?=$recommendedItems->name_store?>
        </a>

        <div class="ps-product__content">

            <a class="ps-product__title" href=" <?=$path . $recommendedItems->url_product?>">
                <?=$recommendedItems->name_product?>
            </a>
                ...
                <div class="ps-product__rating">
                    ...
                    <?php
                        $arrReviews = json_decode($recommendedItems->reviews_product, true);
